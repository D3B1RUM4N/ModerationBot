...
module.exports = {
    ...
    async run(client, message, args, db) {
        ...
        let csv = "ID,Date,Projet,Tache,Description,Time,User\n"
        const workTime = await db.WorkTime.findMany({ where: { serverID: message.guild.id } })
        workTime.forEach(wt => {
            csv += `${wt.workTimeID},${wt.dateWorkTime},${wt.projet},"${wt.task}","${wt.comment}",${wt.workTime},"${client.users.cache.get(wt.userID)?.tag || 'Unknown User'}"\n`
        })
